...
class EditVideosList(Frame):
    def __init__(self, master):
        ...
        refresh_playlist_btn.grid(row=1, column=5, sticky=NW, pady=10)

        add_lbl = tk.Label(self, text="Enter Video Number")
        add_lbl.grid(row=2, column=4)

        self.add = tk.Entry(self, width=3)
        self.add.focus()
        self.add.grid(row=2, column=5, sticky=W, padx=15)
        ...
        delete_lbl.grid(row=4, column=4)

        self.delete = tk.Entry(self, width=3)
        self.delete.grid(row=4, column=5, sticky=W, padx=15)
    ...
    def delete_video_from_playlist_clicked(self):
        try:
            ...
            elif len(self.__playlist_names) == 0:
